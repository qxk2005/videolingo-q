...
- **⚡ 官方字幕极速通道与时序净化**：支持在视频下载时，智能识别并优先通过第三方平台 `downloadyoutubesubtitles.com` 获取 YouTube 官方或自动生成的英文字幕（**完全绕过 WhisperX 语音识别**），并加入**智能时间轴去重合算法 (De-overlapping)** 自动修正重叠的时间线，消除字幕遮挡，实现字幕播放时文字首尾相接、互不重叠，极大提升字幕播放体验。
...
- **YouTube 官方字幕通道 (Subtitle Bypass & De-overlapping)**：
  ...
  - **第三方极速直达**：利用集成的安全解密算法，程序自动通过第三方平台免除了滚屏重复字符直接下载纯净 SRT 字幕。
