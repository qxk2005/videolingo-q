...
def get_batch_split_prompt(items):
    """Batch split prompt for multiple sentences in one LLM call.
    items: list of (sentence, num_parts, word_limit)
    Output JSON keyed "1".."N" each with a "split" field.
    """
    language = load_key("whisper.detected_language")

    sentences_block = ""
    output_spec = {}
    for local_i, (sentence, num_parts, word_limit) in enumerate(items, 1):
        sentences_block += f"  {local_i}. Split into {num_parts} part(s) (each < {word_limit} words): \"{sentence}\"\n"
        br_example = " [br] ".join([f"part{j}" for j in range(1, num_parts + 1)])
        output_spec[str(local_i)] = {"split": br_example}

    json_format = json.dumps(output_spec, indent=2, ensure_ascii=False)

    return f"""## Role
You are a professional Netflix subtitle splitter in **{language}**.

## Task
Split EACH numbered subtitle sentence using [br] tags at the natural split positions.

Rules:
1. Maintain sentence meaning coherence (Netflix subtitle standards)
2. Keep parts roughly equal in length (minimum 3 words each)
3. Split at natural points: punctuation, conjunctions, clause boundaries
4. For repeated words, split at the middle
...
Note: Start your answer with ```json and end with ```, do not add any other text.""".strip()
...
def get_prompt_faithfulness(lines, shared_prompt):
    ...
    for i, line in enumerate(line_splits, 1):
        json_dict[f"{i}"] = {"origin": line, "direct": f"direct {TARGET_LANGUAGE} translation {i}.", "corrected_source": line}
...
4. In "corrected_source": copy the origin text exactly, but fix obvious ASR misrecognition errors in proper nouns, brand names, or technical terms using context clues. If no obvious errors, keep identical to "origin". Do NOT restructure sentences or change normal words.
...
def get_batch_align_prompt(items):
    """Batch alignment prompt for N subtitle pairs in one LLM call.
    items: list of (src_sub, tr_sub, src_part) where src_part uses '\\n' as split marker.
    Output JSON: {"1": {"align": [{"src_part_1": ..., "target_part_1": ...}, ...]}, "2": {...}}
    """
    ...
    items_block = ""
    output_spec = {}
    for local_i, (src_sub, tr_sub, src_part) in enumerate(items, 1):
        src_splits = src_part.split('\n')
        num_parts = len(src_splits)
        src_part_display = src_part.replace('\n', ' [br] ')
        items_block += f"\n  Item {local_i}:\n    {src_lang} Original: \"{src_sub}\"\n    {targ_lang} Original: \"{tr_sub}\"\n    Pre-processed {src_lang} ([br] = split points): {src_part_display}\n"
        align_parts = [{f"src_part_{j+1}": src_splits[j], f"target_part_{j+1}": f"Aligned {targ_lang} part {j+1}"} for j in range(num_parts)]
        output_spec[str(local_i)] = {"align": align_parts}
...
## Task
For EACH numbered item, split the {targ_lang} subtitle to match the pre-processed {src_lang} split positions.

Rules:
1. Analyze word order and structural correspondence between {src_lang} and {targ_lang}
2. Never leave empty lines; rewrite if needed to split meaningfully
...
def get_batch_trim_prompt(items):
    """items: list of (text, duration) tuples needing shortening.
    Returns a prompt that asks the LLM to shorten all subtitles in one call."""
    rule = '''Consider a. Reducing filler words without modifying meaningful content. b. Omitting unnecessary modifiers or pronouns, for example:
    - "Please explain your thought process" can be shortened to "Please explain thought process"
    - "We need to carefully analyze this complex problem" can be shortened to "We need to analyze this problem"
    - "Let's discuss the various different perspectives on this topic" can be shortened to "Let's discuss different perspectives on this topic"
    ...
    items_text = '\n'.join(
        f'{i+1}. Subtitle: "{text}" | Duration: {duration:.2f}s'
        for i, (text, duration) in enumerate(items)
    )
    result_example = ', '.join(f'"{i+1}": {{"result": "shortened subtitle {i+1}"}}' for i in range(min(2, len(items))))

    return f'''## Role
You are a professional subtitle editor shortening subtitles that exceed voiceover duration.

## Processing Rules
{rule}

## INPUT — {len(items)} subtitles to shorten
{items_text}

## Task
For each numbered subtitle, output its shortened version preserving meaning. Respond only in JSON:
```json
{{{result_example}, ...}}
```
Keys are the subtitle numbers (strings). Each value must have a "result" field with the shortened text in the original language.
Note: Start your answer with ```json and end with ```, do not add any other text.'''.strip()
...
def get_vocab_correction_prompt(text, vocab):
    return f'''
## Role
You are an expert ASR (Automatic Speech Recognition) text corrector.

## Task
You are given a transcribed text and a list of domain-specific vocabulary.
Your task is to identify any misrecognized words in the text that should have been the domain vocabulary terms.
Consider phonetically similar words or common ASR errors.
