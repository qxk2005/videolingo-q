...
def gen_dub_chunks():
    ...
    # Use orig_text (pre-trim) for matching so trimmed text doesn't cause mismatch.
    match_col = 'orig_text' if 'orig_text' in df.columns else 'text'
    for idx, row in df.iterrows():
        target = clean_text(row[match_col])
        matches = []
        current = ''
