...
def valid_translate_result(result: dict, required_keys: list, required_sub_keys: list):
    if not isinstance(result, dict):
        return {"status": "error", "message": f"Response is not a dict: {type(result).__name__}"}
    # Check for the required key
    if not all(key in result for key in required_keys):
        return {"status": "error", "message": f"Missing required key(s): {', '.join(set(required_keys) - set(result.keys()))}"}
    
    # Check for required sub-keys in all items, and that translation values are non-empty
    for key in result:
        ...
        for sub_key in required_sub_keys:
            val = result[key].get(sub_key, "")
            if not str(val).strip():
                return {"status": "error", "message": f"Empty translation for item {key}.{sub_key}"}
...
def translate_lines(lines, previous_content_prompt, after_cotent_prompt, things_to_note_prompt, summary_prompt, index = 0):
    ...
    # Extract corrected sources (fallback to origin if field absent)
    corrected_sources = "\n".join([
        str(faith_result[i].get("corrected_source", faith_result[i]["origin"])).replace('\n', ' ').strip()
        for i in faith_result
    ])
    ...
    if not reflect_translate:
        # If reflect_translate is False or not set, use faithful translation directly
        translate_result = "\n".join([str(faith_result[i]["direct"]).strip() for i in faith_result])
        ...
        console.print(table)
        return translate_result, lines, corrected_sources
    ...
    translate_result = "\n".join([str(express_result[i]["free"]).replace('\n', ' ').strip() for i in express_result])
