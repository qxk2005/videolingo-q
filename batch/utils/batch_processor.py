...
def process_batch():
    ...
    # Filter tasks that need to be processed
    tasks_to_process = df[pd.isna(df['Status']) | df['Status'].str.contains('Error', na=False)]
    ...
    with progress:
        overall_task = progress.add_task("[bold cyan]整体批处理进度", total=total_tasks)
        
        for index, row in df.iterrows():
            if pd.isna(row['Status']) or 'Error' in str(row['Status']):
                video_file = row['Video File']
                progress.update(overall_task, description=f"[bold cyan]正在处理: {video_file}")
                ...
                console.print(f"Skipping task: {row['Video File']} - Status: {row['Status']}")
